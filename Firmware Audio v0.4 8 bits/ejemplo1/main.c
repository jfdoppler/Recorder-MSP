...
VERSION 0.4 8 bits
- Se graban solo 8 bits pero se aumenta la frecuencia de adquisción
- Se duplica el tamaño del buffer (175 -> 350) ya que solo es necesario grabar 1 byte por medicion
...
- SECTORES CON ERRORES POR CARGA LENTE
...
#define buffer_size 400 // Tamaño buffer
...
unsigned char sdWrite(unsigned char data)
{
  unsigned char status;
    ...
    {                               //we have reached the end of the current sector
      ...
      if(flashDisk.currentSector-flashDisk.sectorInicial == 399) //record 400 sectors (~10 seconds) of audio
      {
...
int main( void )
{
  ...
  // Seteo el puerto 1.0 como salida logica para controlar un led
  ...
  TACCR0 = 39; // Cuenta hasta TACCR0 + 1 = 40 micro seg -> cada 40us 1 medicion = 25kHz
               // Este valor viene de la prueba de frec adquisición, para valores mayores empieza a haber demasiados errores
  ...
  while(1){
    if(audio.Rec){ // Si estamos en modo grabar
      if(audio.bufferBytes>0){   //There is a byte to be written in the record buffer
        ...
        status=sdWrite(audio.msb[buffer_out]);//Escribo el byte
